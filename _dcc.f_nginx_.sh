...
#---------------------------------------
    ...
    #_________________________
    # Copiar Ficheiros necessário para o Serviço
    echo "2) Copiar Ficheiros necessário para o Serviço"
    cd /root/nginx
    ...
    #_________________________
    # Criar Directorias necessárias
    echo "3) Criar Directorias necessárias"
    mkdir -p /etc/nginx
...
--without-http_ssi_module --without-http_uwsgi_module --without-http_scgi_module
    ...
    #_________________________
    # Configurar serviço e activo
    echo "6) Configurar serviço e activo"
...
#---------------------------------------
# Faz o Update do directório Source do
# Nginx em /root/nginx
#---------------------------------------
    clear
    echo
    echo "A fazer update do directório /root/nginx ..."
    echo
...
#---------------------------------------
    ...
    then
        if [ -z "$2" ]
        then
            echo "é preciso especificar o VHOST, assim:"
        ...
        if [ "$RESULT" == "" ]; then
            echo "Não existe neste servidor!!"
            exit 0
        fi
    else
...
#----------------------------------------------------
    ...
    echo
    if [ ! -d "/root/nginx/mod_rpaf-0.6" ]
    then
        echo "/root/nginx/mod_rpaf-0.6 NAO ENCONTRADO!!!!"
        echo "Por favor faÃ§a upload do mod_rpaf para /root/nginx/mod_rpaf-0.6"
        echo
        echo -e "... $RED[ FAILED ]$RESET"
    else
        if [ ! -f "/usr/local/apache/modules/mod_rpaf-2.0.so" ]
        then
...
#----------------------------------------------------
    ...
    cecho "RPAF Instalar/Update..." $boldyellow
    echo
    if [ ! -d "/root/nginx/mod_cloudflare" ]
    then
        echo "/root/nginx/mod_cloudflare NAO ENCONTRADO!!!!"
        echo "Por favor faÃ§a upload do mod_cloudflare para /root/nginx/mod_cloudflare"
        echo
        echo -e "... $RED[ FAILED ]$RESET"
    else
        if [ ! -f "/usr/local/apache/modules/mod_cloudflare.so" ]
        then
        ...
        echo "/root/nginx/mod_cloudflare NAO ENCONTRADO!!!!"
           echo "ERRO!!! /usr/local/apache/modules/mod_cloudflare.so já existe"
           echo
           echo -e "... $RED[ FAILED ]$RESET"
        fi
        cd /root/nginx
